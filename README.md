...
- **HTML 결과 페이지**: 보기 좋은 웹 페이지로 출력 (다크모드, 반응형 디자인)
...
```bash
git clone https://github.com/<username>/hsk-analyzer.git
cd hsk-analyzer
```
...
# HSK 5~6급 VOCA 분석
python run.py --source voca_56 --all

# GPT 모델 변경 (기본: gpt-4o-mini)
python run.py --model gpt-5-mini --track TRACK001.mp3

# 캐시 무시하고 재분석
python run.py --force --track TRACK001.mp3
```

분석 완료 후 `output/index.html`을 브라우저에서 열면 결과를 확인할 수 있습니다.
...
| 설정 | 기본값 | 설명 |
|------|--------|------|
| `GPT_MODEL` | `gpt-4o-mini` | GPT 모델 (gpt-5-mini, gpt-4.1-nano 등) |
...
듣기 트랙 전체(~95개) 분석 시 약 $7~12 정도 소요됩니다 (gpt-4o-mini 기준).

## HTML 페이지 기능

- **오디오 플레이어**: 배속 조절 (0.5x ~ 1.25x), 5초 되감기/앞으로
